...
# -- =============================================================================
# --- by 5 random choice
def by_5_ran(min_num, max_num):
    int_out = random.choice([i for i in list(range(min_num, max_num + 1)) if i%5 == 0])
    return(str(int_out))

# --- primary function - returns workout
def get_workout(wo_type, wo_type_not, rest_chance, rest_chance_mod):
    ...
    # --- all workout lists
    ub_dic = {'merkins':[10,40], 'burpees':[5,30], 'coup overhead press':[10,40], 'coup curls':[10,60], 'dips':[10,60], 'coup skull crushers':[10,50], 'coup rows':[10,60], 'cat wine mixers':[10,40], 'jumping spiders':[5,30]}
    lb_dic = {'squats':[20,60], 'split lunges':[10,30], 'coup thrusters':[5,25], 'box jump':[10,30], 'step ups':[20,60], 'coup swings':[15,30], 'alt side squats':[20,80], 'bobby hurleys':[20,60]}
    cb_dic = {'big boy situps':[10,60], 'starfish crunches':[10,40], 'flutter kicks':[20,60], 'mountain climbers':[15,60], 'american hammers':[15,100], 'bycicles':[15,80]}
    all_dic = dict(ub_dic)
    ...
    if(wo_type == 'upper'):
        ...
        # --- get workout amount
        str_out_02 = by_5_ran(ub_dic[str_out_01][0], ub_dic[str_out_01][1])
        # --- final output
        str_out = str_out_01 + '\nx' + str_out_02
    ...
    if(wo_type == 'lower'):
        ...
        # --- get workout amount
        str_out_02 = by_5_ran(lb_dic[str_out_01][0], lb_dic[str_out_01][1])
        # --- final output
        str_out = str_out_01 + '\nx' + str_out_02
    ...
    if(wo_type == 'core'):
        ...
        # --- get workout amount
        str_out_02 = by_5_ran(cb_dic[str_out_01][0], cb_dic[str_out_01][1])
        # --- final output
        str_out = str_out_01 + '\nx' + str_out_02
    ...
    if(wo_type == 'all'):
        ...
        # --- get workout amount
        str_out_02 = by_5_ran(all_dic[str_out_01][0], all_dic[str_out_01][1])
        # --- final output
        str_out = str_out_01 + '\nx' + str_out_02
